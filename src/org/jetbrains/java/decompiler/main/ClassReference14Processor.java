...
public class ClassReference14Processor {
	...
	static {
		...
		NewExprent newExpr = new NewExprent(new VarType(CodeConstants.TYPE_OBJECT, 0, "java/lang/NoClassDefFoundError"), new ArrayList<>(),
				null);
		...
		invCause.setLstParameters(Collections
				.singletonList(new VarExprent(2, new VarType(CodeConstants.TYPE_OBJECT, 0, "java/lang/ClassNotFoundException"), null)));
			...
			if ((noSynthFlag || mt.isSynthetic()) && mt.getDescriptor().equals("(Ljava/lang/String;)Ljava/lang/Class;")
				...
				if (root != null && root.getFirst().type == Statement.TYPE_TRYCATCH) {
					...
					if (cst.getStats().size() == 2 && cst.getFirst().type == Statement.TYPE_BASICBLOCK
							&& cst.getStats().get(1).type == Statement.TYPE_BASICBLOCK && cst.getVars().get(0).getVarType()
									.equals(new VarType(CodeConstants.TYPE_OBJECT, 0, "java/lang/ClassNotFoundException"))) {
